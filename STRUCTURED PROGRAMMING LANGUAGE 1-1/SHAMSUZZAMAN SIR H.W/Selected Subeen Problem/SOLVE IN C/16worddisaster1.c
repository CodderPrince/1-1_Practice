    ...
    while(t--)
    {
        ...
        if(j>0)
         /*jokhon only single ekta word thakbe tokhon ei loop
          ...
          muloto jodi prince ei word input dei only loop cholbe 
          prince porjonto kintu loop er baire ar cholte parbe na
          bole loop thekei ber hoye jabe karon pore ar kono space
          pabe na karon already string er loop shes hoye gese tai
          loop er baire chole eshe ei condition check korbe ar 
          print korbe*/
        {
            re[j]='\0';
            reverse(re);
...
int reverse (char receiver_re[101])
